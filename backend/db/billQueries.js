...
class BillQueries {
        ...
        const result = await pool.request()
            ...
            .query(`
                SELECT b.bill_id, b.prescription_id, b.patient_id, b.doctor_id,
                       ...
                       pd.quantity,
                       10.00 AS unit_price,
                       (pd.quantity * 10.00) AS item_total
                FROM bills b
                JOIN patients p ON b.patient_id = p.patient_id
                JOIN doctors d ON b.doctor_id = d.doctor_id
        ...
        const result = await pool.request()
            ...
            .query(`
                SELECT b.bill_id, b.prescription_id, b.doctor_id,
                       ...
                       ISNULL(SUM(pd.quantity), 0) AS total_quantity
                FROM bills b
                JOIN doctors d ON b.doctor_id = d.doctor_id
                LEFT JOIN prescription_details pd ON b.prescription_id = pd.prescription_id
                WHERE b.patient_id = @id
                GROUP BY b.bill_id, b.prescription_id, b.doctor_id, b.bill_date, b.total_amount, b.status, d.name
                ORDER BY b.bill_date DESC
            `);
        ...
        const result = await pool.request()
            ...
            .query(`
                SELECT d.doctor_id,
                       ...
                       ISNULL(SUM(pd.quantity), 0) AS total_medicines_prescribed,
    ...
    async getMonthlyBillReport() {
        ...
        const result = await pool.request().query(`
            SELECT YEAR(b.bill_date) AS year,
                   ...
                   ISNULL(SUM(pd.quantity), 0) AS total_medicines_issued,
                   ISNULL(SUM(b.total_amount), 0) AS total_revenue
            FROM bills b
            LEFT JOIN prescription_details pd ON b.prescription_id = pd.prescription_id
    ...
    async updateBillStatus(billId, status) {
        ...
        const result = await pool.request()
            ...
            .query(`
                UPDATE bills SET status = @status WHERE bill_id = @id
            `);
    ...
    async getBillByPrescription(prescriptionId) {
        ...
        const result = await pool.request()
            .input("id", sql.Int, prescriptionId)
            .query(`SELECT * FROM bills WHERE prescription_id = @id`);
