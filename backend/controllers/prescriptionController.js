...
async getByPatient(req, res) {
    try {
        ...
        const rows =
            await prescriptionQueries.getPrescriptionsByPatient(
                patientId
            );
...
for (const item of details) {
    ...
    if (!medicine) {
        return res.status(404).json({
            error: `Medicine ID ${item.medicine_id} not found`
        });
    }
...
async update(req, res) {
    try {
        ...
        if (!patient_id || !doctor_id || !date) {
            return res.status(400).json({
                error: 'Missing required fields: patient_id, doctor_id, date'
            });
        }

        const result =
            await prescriptionQueries.updatePrescription(
                id,
                patient_id,
                doctor_id,
                date
...
async delete(req, res) {
    try {
        ...
        const result =
            await prescriptionQueries.deletePrescription(id);
...
async getBillTotal(req, res) {
    try {
        ...
        const result =
            await prescriptionQueries.getPrescriptionBillTotal(id);
        ...
        res.status(500).json({
            error: 'Failed to get bill total'
        });
    }
}
}
