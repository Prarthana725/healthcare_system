...
export default function DoctorPanel() {
    ...
    // Grab the exact login name the system sees
        ...
        async function initDoctorProfile() {
            try {
                ...
                if (res.ok) {
                    ...
                    // Try to match the database 'user_id' text to your exact login name
                    const myDoc = allDocs.find(d => d.user_id === loginName);
                    
                    if (myDoc) {
                        setMyDoctorId(myDoc.doctor_id || myDoc.id);
                        setMessage(''); // Clear warning if found
                    } else {
                        // DIAGNOSTIC WARNING: Tells you exactly what is wrong
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#f1f5f9', padding: '30px', fontFamily: "'Segoe UI', sans-serif" }}>
            ...
            {/* STATS */}
            <div style={statsGrid}>
                <div style={cardStyle}><div style={cardTitle}>📅 Appointments</div><div style={cardValue}>{appointments.length}</div></div>
                <div style={cardStyle}><div style={cardTitle}>✅ Completed</div><div style={cardValue}>{completedAppointments}</div></div>
                <div style={cardStyle}><div style={cardTitle}>💊 Medicines Available</div><div style={cardValue}>{medicines.length}</div></div>
            </div>
                        ...
                        <tbody>
                            {appointments.map((a) => (
                                <tr key={a.appointment_id || a.id}>
                                    ...
                                    <td style={{...tableData, color: a.status?.toLowerCase() === 'completed' ? '#16a34a' : '#ea580c', fontWeight: 'bold'}}>{a.status || 'Pending'}</td>
                                    <td style={tableData}>
                                        {a.status?.toLowerCase() !== 'completed' && (
                                            <button onClick={() => updateAppointmentStatus(a.appointment_id || a.id, 'Completed')} style={btnStyle}>Mark Complete</button>
                                        )}
                                    </td>
                                </tr>
                            ))}
            ...
            <div style={{ ...panelStyle, marginTop: '30px' }}>
                ...
                <form onSubmit={handlePrescriptionSubmit}>
                    ...
                    <button type="button" onClick={addMedicineRow} style={secondaryBtn}>+ Add Medicine</button>
                    <br /><br />
                    <button type="submit" style={submitBtn}>Create Prescription</button>
                </form>
...
const debugBadge = { marginLeft: '15px', background: 'rgba(0,0,0,0.3)', padding: '6px 12px', borderRadius: '8px', fontSize: '14px', fontWeight: 'bold' };
...
const messageStyle = { marginTop: '20px', padding: '16px', borderRadius: '10px', fontWeight: 'bold', border: '1px solid #fca5a5' };